...
> **DONE**; only `IRPtrOffset` remains of the original quintet. New tally:
> **11 DONE / 4 GAP / 1 N/A**, matching ADR 0003 §D4. Mirrors PRD v4
...
**Tally (HEAD `7915299`):** **11 DONE** (`IRBinOp`, `IRICmp`, `IRSelect`,
`IRRet`, `IRCast`, `IRBranch`, `IRPhi`, `IRAlloca` [static+dynamic],
`IRStore`, `IRLoad`, `IRVarGEP`), **4 GAP** (`IRInsertValue`,
`IRExtractValue`, `IRPtrOffset`, `IRCall`), **1 N/A** (`IRSwitch`,
frontend-pre-expanded). The 2026-05-28 audit recorded 6 DONE / 9 GAP; the
delta is `IRCast` (ADR 0013) plus four of the five memory-quintet opcodes
(`IRAlloca`/`IRStore`/`IRLoad`/`IRVarGEP`; ADR 0009/0014) landing between
`e9dfd7f` and HEAD. The remaining gaps are **`IRPtrOffset`** (last of the
memory quintet), the **aggregates** (`IRExtractValue`/`IRInsertValue`),
and **`IRCall`**.
